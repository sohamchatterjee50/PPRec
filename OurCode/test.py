...
Just test for the data code, and the news encoder atm
...
def main():
    ...
    news_encoder_config = NEConfig(
        n_attention_heads=n_attention_heads, head_output_size=head_output_size
    )

    lookup_news_encoders = [
        LookupNewsEncoder(
            model=model, config=news_encoder_config, device=torch.device("cpu")
        )
