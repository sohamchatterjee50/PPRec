...
def AttentivePoolingQKY(dim1,dim2,dim3):
    ...
    value_input = tf.keras.Input(shape=(dim1,dim3),dtype='float32')
    user_vecs = tf.keras.layers.Dropout(0.2)(vecs_input)
    user_att = tf.keras.layers.Dense(200,activation='tanh')(user_vecs)
    user_att = tf.keras.layers.Flatten()(tf.keras.layers.Dense(1)(user_att))
    user_att = tf.keras.layers.Activation('softmax')(user_att)
    user_vec = tf.keras.layers.Dot((1,1))([value_input,user_att])
    model = tf.keras.Model([vecs_input,value_input],user_vec)
    return model
...
class Attention(layers.Layer):
    ...
    def call(self, x):
        ...
        O_seq = K.permute_dimensions(O_seq, (0,2,1,3,4))
